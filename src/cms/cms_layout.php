...
require_once($COCKATOO_ROOT.'wwwutils/core/reqparser.php');
...
if ( strcmp($op,'fix') === 0) {
  $layout = json_decode($_POST['data'],true);
  if ( $layout ) {
    $brl = brlgen(Def::BP_LAYOUT,$SERVICE,$DEVICE,$PATH,Beak::M_GET);
    $ret = BeakController::beakQuery(array($brl));
    $data = $ret[$brl];
    if ( $data ) {
      $brl = brlgen(Def::BP_LAYOUT,$SERVICE,$DEVICE,$PATH,Beak::M_DEL);
      $ret = BeakController::beakQuery(array($brl));
      $brl = brlgen(Def::BP_LAYOUT,$SERVICE,$DEVICE,$PATH,Beak::M_SET);
      $data[Def::K_LAYOUT_LAYOUT] = $layout;
      BeakController::beakQuery(array(array($brl,$data)));
    }
  }
  return;
}
